...
def detect_pose_image(image_path):
    ...
    # print("1", pose_results)
    # print("2", pose_results.pose_landmarks)
    # print("3", pose_results.pose_landmarks.landmark)
    ...
    # standing_pose_sequence = pose_results
...
def compute_mse(ref_angles, live_angles):
    sum = 0
    # for ref, live in zip(ref_angles, live_angles):
    #     sum += (ref-live)**2
    # print("A")
    # print(sum)
    ...
    # for ref, live in zip(ref_angles, live_angles):
    #     sum += ((np.pi - abs(ref-live))/np.pi)
    # return sum/8
# def check_passing(acc_val):
#     if(acc_val > 90):
...
def practice_loop(live_video, pose, mp_pose, mp_drawing, pass_count): 
    if pass_count < 6:
        # countdown = 3
        # while countdown > 0:
        #     cv2.putText(live_video, str(countdown), (100, 100), cv2.FONT_HERSHEY_COMPLEX,1, (255,0,0),2, cv2.LINE_8)
        #     time.sleep(1)
        #     countdown -= 1
